...
# In-memory candidate store (replace with DB in production)
candidate_store = []
...
def submit_candidate(data: CandidateSubmit):
    ...
    candidate = {
        ...
        "matched_skills": target_match["matched_skills"] if target_match else [],
        "gap_skills": target_match["gap_skills"] if target_match else [],
        "parsed": parsed,
        "scores": scores,
    }
...
def dashboard():
    ...
    avg_score = round(sum(c["ats_score"] for c in candidate_store) / total, 1) if total else 0

    # Skill frequency
    all_skills = []
    for c in candidate_store:
        all_skills.extend(c["parsed"].get("skills", []))
    skill_freq = {}
    for s in all_skills:
    ...
    return {
        ...
        "skill_frequency": [{"skill": s, "count": c, "pct": round(c/total*100) if total else 0} for s, c in top_skills],
        "pipeline": {
            "all": total,
...
def list_candidates(job_id: str = None, min_ats: int = 0):
